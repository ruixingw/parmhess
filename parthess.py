...
finalxyz = ''
for atom in mole:
    hprimexyz += atom.atomsym + '-' + atom.name + '-' + '{:8.6f}'.format(float(
        atom.atomcharge)) + '   ' + '    '.join(
            ["{: .12f}".format(x) for x in atom.coords]) + '\n'
    hessxyz += atom.atomsym + '-' + atom.name + '-0.000000' + '   ' + '    '.join(
        ["{: .12f}".format(x) for x in atom.coords]) + '\n'
    finalxyz += atom.atomsym + '-' + atom.atomtype + '-' + '{:8.6f}'.format(
...
for dihd in mole.dihdlist.values():
    for dihdfunc in mmcom.nozomudihdfunc:
        if matchdihd(dihd, dihdfunc):
            ...
            dihd.forceconst = [rxmol.dihdforceconst(x, dihd)
...
for dihd in mmcom.nozomudihdfunc:
    ...
    for item in dihd.forceconst:
        if str(item) == 'XXXXXX':
            parm.append('0.000')
            logging.critical('Force constant is not determined for dihedral ' +
                             dihd.repr)
            raise
        else:
